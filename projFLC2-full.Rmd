...
markers <- markers %>%
  filter(avg_log2FC > 0) %>%
  write_csv(paste0("./fullDataset/markers", proj_name, '_HighlyEnrichedMarkers_logFC.5_wilcox-12cluster.csv'))
markers <- read_csv(paste0("./fullDataset/markers", proj_name, '_HighlyEnrichedMarkers_logFC.5_wilcox-12cluster.csv'))
head(markers)
```
...
geneset_marker_overlap.f(harmonized.seurat,
                         ...
                         cell_markers,
                         FC_thresh = .5, 
                         projname = paste0("./fullDataset/cell_type_assignment/", proj_name, '_CTA_Macparland_13clusters'))
...
geneset_marker_overlap.f(harmonized.seurat,
                         ...
                         flc,
                         FC_thresh = .5, 
                         projname = paste0("./fullDataset/cell_type_assignment/", proj_name, '_CTA_FLC_13clusters'))
```
...
ggsave(paste0("./fullDataset/cell_type_assignment/", proj_name, "_DP-CTA_Macparland-module.png"), units = "in", width = 2 + length(goi_li) * .75, height = 8)
...
ggsave(paste0("./fullDataset/cell_type_assignment/", proj_name, "_DP-CTA_FLC.png"), units = "in", width = 3 + length(goi_li) * .75, height = 8)
```
...
saveRDS(seurat_obj, paste0("./full" proj_name, '_seurat-obj.rds'))
```
